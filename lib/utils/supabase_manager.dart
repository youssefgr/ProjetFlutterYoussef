...
class SupabaseManager {
  ...
  // =============================================================
// 🔄 MÉTHODE UTILITAIRE POUR FORCER LA RECONNEXION
// =============================================================
  static Future<bool> forceGoogleReauth(BuildContext context) async {
    debugPrint('🔄 Forcer la reconnexion Google...');

    try {
      // 1. Déconnexion complète
      await client.auth.signOut(scope: SignOutScope.global);

      // 2. Nouvelle authentification
      await signInWithGoogle(context);

      // 3. Attendre un peu que l'authentification se fasse
      await Future.delayed(const Duration(seconds: 3));

      // 4. Vérifier si ça a fonctionné
      final session = client.auth.currentSession;
      if (session?.providerToken != null) {
        debugPrint('🟢 Reconnexion Google réussie');
        return true;
      } else {
        debugPrint('❌ Échec reconnexion Google');
        return false;
      }
    } catch (e) {
      debugPrint('❌ Erreur lors de la reconnexion forcée: $e');
      return false;
    }
  }
}
